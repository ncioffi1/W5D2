...
def populous_country_counts
  ...
  SELECT
    continent, COUNT(populous_countries.continent)
  FROM
    (SELECT
      *
    FROM
      countries
    WHERE
      population >= 10000000) AS populous_countries
  GROUP BY
    continent;
  SQL
end
  ...
  execute(<<-SQL)
    ...
    FROM
      ...
      FROM
        countries
      GROUP BY
        continent) AS populous_countries
    WHERE
      populous_countries.pop_sum > 100000000;
  SQL
end
